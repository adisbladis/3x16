...
void x16_hash(const char* input, char* output)
{
    ...
    sph_luffa512 (&ctx_luffa1, input, 64);
    sph_luffa512_close (&ctx_luffa1, hashA);	

    xor_hash(hashA, hashB)

    sph_cubehash512_init (&ctx_cubehash1); 
    sph_cubehash512 (&ctx_cubehash1, input, 64);   
    sph_cubehash512_close(&ctx_cubehash1, hashA);  

    xor_hash(hashA, hashB)

    sph_shavite512_init (&ctx_shavite1);
    sph_shavite512 (&ctx_shavite1, input, 64);   
    sph_shavite512_close(&ctx_shavite1, hashA);  

    xor_hash(hashA, hashB)

    sph_simd512_init (&ctx_simd1); 
    sph_simd512 (&ctx_simd1, input, 64);   
    sph_simd512_close(&ctx_simd1, hashA); 

    xor_hash(hashA, hashB)

    sph_echo512_init (&ctx_echo1); 
    sph_echo512 (&ctx_echo1, input, 64);   
    sph_echo512_close(&ctx_echo1, hashA); 

    xor_hash(hashA, hashB)

    sph_hamsi512_init (&ctx_hamsi1);
    sph_hamsi512 (&ctx_hamsi1, input, 64);
